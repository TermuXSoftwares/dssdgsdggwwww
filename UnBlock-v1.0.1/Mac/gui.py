...
AUTOSTART_PLIST = Path.home() / "Library" / "LaunchAgents" / "com.unblock.app.plist"
...
def set_autostart_enabled(enabled: bool) -> bool:
    try:
        if enabled:
            AUTOSTART_PLIST.parent.mkdir(parents=True, exist_ok=True)
...
"""
            AUTOSTART_PLIST.write_text(content, encoding="utf-8")
            subprocess.run(["launchctl", "load", str(AUTOSTART_PLIST)], capture_output=True)
        else:
            if AUTOSTART_PLIST.exists():
                subprocess.run(["launchctl", "unload", str(AUTOSTART_PLIST)], capture_output=True)
                AUTOSTART_PLIST.unlink()
        return True
    except Exception:
        return False
